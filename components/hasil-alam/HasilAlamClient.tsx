...
/** Shared product card inner content */
function ProductCard({ product }: { product: Product }) {
    const href = product.slug ? `/hasil-alam/${product.slug}` : "#";
        ...
        <Link href={href} className="block">
            ...
            <div className="mt-2.5">
                ...
                <div className="mt-2 flex gap-1.5 flex-wrap">
                    {product.shopeeUrl && <span className="rounded bg-[#EE4D2D]/10 px-2 py-0.5 font-body text-[10px] font-medium text-[#EE4D2D]">🟠 Shopee</span>}
                    {product.tiktokUrl && <span className="rounded bg-urbane-bronze/10 px-2 py-0.5 font-body text-[10px] font-medium text-urbane-bronze">🎵 TikTok</span>}
                    {product.lazadaUrl && <span className="rounded bg-[#0F146D]/10 px-2 py-0.5 font-body text-[10px] font-medium text-[#0F146D]">🔵 Lazada</span>}
                    {product.tokopediaUrl && <span className="rounded bg-[#42B549]/10 px-2 py-0.5 font-body text-[10px] font-medium text-[#42B549]">🟢 Tokopedia</span>}
                </div>
                <div className="mt-2 w-full rounded-[4px] border border-terracotta-earth/30 py-2 text-center font-body text-xs font-medium text-terracotta-earth shadow-sm transition-all group-hover:bg-terracotta-earth group-hover:text-white lg:text-sm">
                    Lihat Detail
                </div>
            </div>
        </Link>
            ...
            {filtered.length <= 2 ? (
                ...
                <div className="mt-8 flex flex-wrap justify-center gap-6 sm:gap-8">
                    {filtered.map((product) => (
                        <StaggerItem
                            key={product._id}
                            className="group w-full max-w-[280px] cursor-pointer rounded-lg transition-shadow hover:shadow-lg p-2 -m-2"
                            direction="up"
                        >
                    ...
                    {/* Coming Soon Teaser */}
                    <StaggerItem
                        className="w-full max-w-[280px] flex-shrink-0 rounded-lg border-2 border-dashed border-eucalyptus-calm/30 bg-eucalyptus-calm/5 p-2"
                        direction="up"
                    >
                        <div className="flex aspect-square flex-col items-center justify-center gap-3 rounded-lg">
                            ...
                            <p className="px-4 text-center font-body text-xs text-text-secondary leading-relaxed">Produk baru sedang dalam persiapan. Pantau terus!</p>
                        </div>
                    </StaggerItem>
                </div>
            ) : (
